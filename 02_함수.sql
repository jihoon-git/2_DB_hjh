...
-- EMPLOY 테이블에서 사번이 짝수인 사원의 사번, 이름 조회
SELECT EMP_ID, EMP_NAME
FROM EMPLOYEE
WHERE MOD(EMP_ID, 2)=0;

-- EMPLOY 테이블에서 사번이 홀수인 사원의 사번, 이름 조회
SELECT EMP_ID, EMP_NAME
FROM EMPLOYEE
...
SELECT ROUND(123.456, -1) FROM DUAL; --120, 소수점 0번째 자리에서 반올림해서
									-- 소수점 -1자리 표현
									-- == 1의 자리에서 반올림 해서 10 자리부터 표현

SELECT ROUND(123.456, -2) FROM DUAL; --10의 자리에서 반올림 해서 100 자리부터 표현
...
SELECT TRUNC(123.456, -1) FROM DUAL; -- 120, 10의 자리(소수점 -1) 자리 아래 버림
...
-- 직원의 급여를 인상하고자 한다.
-- 직금 코드가 J7인 직원은 20% 인상,
-- 직금 코드가 J6인 직원은 15% 인상,
-- 직금 코드가 J5인 직원은 10% 인상,
-- 그 외 직급은 5% 인상
...
SELECT DISTINT DEPT_CODE FROM EMPLOYEE; -- 7행

SELECT COUNT(DISTINT DEPT_CODE) FROM EMPLOYEE; -- 6행
