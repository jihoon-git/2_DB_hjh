...
SELECT DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여') 성별,
	...
	SUM(SALARY) "급여 합"
FROM EMPLOYEE
GROUP BY DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여') -- 별칭 사용X (SELECT절 해석X)
...
FROM EMPLOYEE
WHERE
EXTRACT (YEAR FROM HIRE_DATE) >= 2000 -- 가장 효율적
-- HIRE_DATE >= TO_DATE('2000-01-01')
-- SUBSTR(HIRE_DATE, 1, 4) >=2000 -- 형변환 2회
GROUP BY JOB_CODE
ORDER BY 1;
...
FROM EMPLOYEE
--WHERE SALARY >= 3000000 --> 한 사람의 급여가 300만 이상이라는 조건(요구사항 충족X)
GROUP BY DEPT_CODE
HAVING AVG(SALARY) >= 3000000 -> DEPT_CODE 그룹 중 급여 평균이 3백만 이상인 그룹만 조회
ORDER BY DEPT_CODE;

-- EMPLOYEE 테이블에서 직급별 인원수가 5명 이하인 직급코드, 인원수를 조회
...
-- ROLLUP : GROUP BY 절에서 가장 먼저 작성된 컬럼의 중간 집계를 처리 하는 함수
...
-- CUBE : GROUP BY 절에 작성된 모든 컬럼의 중간 집계를 처리 하는 함수
...
-- 서로 다른 테이블이지만 컬럼의 타입, 개수만 일치하면 집합 연선자 사용 가능
